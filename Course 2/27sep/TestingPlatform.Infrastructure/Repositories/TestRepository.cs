...
namespace TestingPlatform.Infrastructure.Repositories
{
    public class TestRepository : ITestRepository
    {
            ...
            )
        {
            ...
            // Базовый запрос к таблице тестов
            var tests = _context.Tests
                .AsNoTracking()
                .AsQueryable();

            // Фильтруем по публичности, если передан параметр
            if (isPublic is not null)
                tests = tests.Where(t => t.IsPublic == isPublic);

            // Фильтруем по группам, если передан список groupIds
            if (groupIds != null && groupIds.Any())
                tests = tests.Where(t => t.Groups.Any(g => groupIds.Contains(g.Id)));

            // Фильтруем по студентам, если передан список studentIds
            if (studentIds != null && studentIds.Any())
                tests = tests.Where(t => t.Students.Any(s => studentIds.Contains(s.Id)));

            // Сортировка по выбранному полю
            tests = sortBy?.ToLower() switch
            {
                "title" => sortDescending ? tests.OrderByDescending(t => t.Title) : tests.OrderBy(t => t.Title),
                "publishedat" => sortDescending ? tests.OrderByDescending(t => t.PublishedAt) : tests.OrderBy(t => t.PublishedAt),
                _ => tests.OrderByDescending(t => t.PublishedAt).ThenBy(t => t.Title)
            };
